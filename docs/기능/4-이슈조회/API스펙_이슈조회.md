...
- 작성: 채윤성 / 상태: 검토 초안 / 상대 컴포넌트: [mint-web W1](https://github.com/MEV-SW/mint-web/issues/11)
- 명칭: 산업 사건은 `story`로 부르며 개발 작업 이슈와 구별한다. 아래 경로·필드는 신규 계약 제안이다.

## 엔드포인트 목록
| Method | Path | 용도 | 인증 |
|---|---|---|---|
| GET | /api/v1/stories | 공개 근거가 있는 이슈 목록 | 기존 Bearer JWT, 활성 사용자 |
| GET | /api/v1/stories/{story_id} | 요약·근거 상세 | 동일 |

## 상세
### 공통 계약
- 기존 [개인화 API](https://github.com/MEV-SW/mint-server/blob/main/app/api/v1/personalization.py)와 [예외 형식](https://github.com/MEV-SW/mint-server/blob/main/app/core/exceptions.py)을 따른다.
- JWT에서 사용자·조직을 결정한다. request에서 조직을 지정하지 않는다.
- 기존 게시물 공개 및 MembershipService 권한을 목록·검색·요약·출처 개수에 모두 적용한다. 범위 밖 근거를 제거한 결과 읽을 수 있는 기사가 없으면 해당 이슈를 노출하지 않는다.
- 요약은 공개 가능한 근거만으로 만든다. 접근 제한 자료가 섞인 저장 요약은 재사용하지 않고 `summary=null`, `summary_status="unavailable"`로 반환한다.
- UUID 식별자는 문자열, 모든 시각은 UTC ISO 8601이다. 웹은 Asia/Seoul로 표시한다.
- feature flag 제안: `ISSUE_RADAR_ENABLED`, 기본 OFF. OFF이면 두 API 모두 403 `{"detail":"이슈 레이더 기능이 비활성화되어 있습니다."}`. 기존 API는 변경하지 않는다.
- 인증 없음·만료 401, 비활성 계정 등 기존 접근 거절 403, 파라미터 검증 오류 422는 기존 FastAPI 형식, 일시적 조회 실패 503 `{"detail":"잠시 후 다시 시도해 주세요."}`.
- 기존 뉴스·토픽·리포트 response는 변경하지 않는다. 변화 이력·추적 mutation·관리자 교정은 다른 카드의 별도 계약이다.
...
- 오류: 위 공통 오류. 존재하지 않거나 사용자 범위 밖 category ID는 200 빈 결과를 반환해 존재 여부를 노출하지 않는다.
- flag: 공통 정책 적용.
